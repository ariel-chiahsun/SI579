    ...
    datamuseRequest(getDatamuseRhymeUrl(wordInput.value), (data) => {
        ...
        if (data.length==0){
            ...
            for (let [key, group] of Object.entries(groups)) {
                ...
                for(let i in group){
                    const {word} = group[i]
                    //console.log(numSyllables,word)
                    //console.log(word);
                    wordOutput.lastChild.innerHTML +=  `<li>${word} <button id=${word} type="button" class="btn-outline-success" onclick="addToSavedWords(this.id)">(Save)</button></li>`;
                }
            }
...
showSynonymsButton.addEventListener('click', ()=>{
    console.log('clicked show synonyms button');
    outputDescription.innerHTML ='';
    wordOutput.innerHTML ='';
    datamuseRequest(getDatamuseSimilarToUrl(wordInput.value), (data) => {
        ...
        if (data.length==0){
            ...
            for (let i in data){
                let {word} = data[i]
                wordOutput.innerHTML +=  `<li>${word} <button id=${word} type="button" class="btn-outline-success" onclick="addToSavedWords(this.id)">(Save)</button></li>`;
            }
